...
public class ClientRunnable implements Runnable {
    ...
    private void checkForInitialization() {
        ...
        if (messageIter.hasNext()) {
            ...
            System.out.println("checkForInitialization msg: " + msg);

            String name = msg.getName();
            System.out.println("name: " + name);
            String checkRet = Server.checkUserNameValid(name);
            System.out.println("name: " + name + " check ret: " + checkRet);
            if (checkRet.equals("")) {
                if (setUserName(name)) {
                    System.out.println("connect successfully.");
                    // send back ack
                    // If the message is a connect message, make a response. Actually, when init, it must be connect msg.
                    if (msg.getType().equals(MessageType.CONNECT_MESSAGE)) {
                        // Reply with success.
                        enqueueMessage(Message.makeConnectResponse(name, "true", "There are " + (Server.currentActiveNum() - 1) + " other connected clients"));
                ...
                System.out.println("Should reply false for this connect message and terminate.");
                // Reply with fail.
                enqueueMessage(Message.makeConnectResponse(name, "false", checkRet));
                handleOutgoingMessages();
                initialized = false;
            ...
            System.out.println("checkForInitialization initialized:" + initialized);
        }
    }
    ...
    protected void handleIncomingMessages() {
        // Client has already been initialized, so we should first check if there are any input messages.
        Iterator<Message> messageIter = connection.iterator();
        ...
        if (messageIter.hasNext()) {
            ...
            if (msg.terminate()) {
                ...
                if (messageChecks(msg)) {
                    // Do something according to the msg type.
                    if (msg.isBroadcastMessage()) {
                        System.out.println("tt received broadcast msg: " + msg);
                        Server.broadcastMessage(msg);

                    } else if (msg.isDirectMessage()) {
                        System.out.println("tt received direct msg: " + msg);
                        String recipient = msg.getRecipient();
                        if (Server.checkUseExist(recipient)) {
                            Server.directMessage(msg);
                        } else {
                            enqueueMessage(Message.makeFailedMessage(msg.getName(), recipient + " not exists."));
                        }

                    } else if (msg.isQueryMessage()) {
                        System.out.println("tt received query msg: " + msg);
                        enqueueMessage(Message.makeQueryResponseMessage(msg.getName(), Server.getConnectedUserList(msg.getName())));

                    } else if (msg.isSendInsultMessage()) {
                        System.out.println("tt received insult msg: " + msg);
                        String recipient = msg.getRecipient();
                        if (Server.checkUseExist(recipient)) {
                            String insult = msg.getText();
                            String sender = msg.getName();
                            Server.broadcastMessage(Message.makeBroadcastMessage(sender, sender + " -> " + recipient + " : " + insult));
                        } else {
                            enqueueMessage(Message.makeFailedMessage(msg.getName(), recipient + " not exists."));
                        }
    ...
    protected void handleOutgoingMessages() {
        ...
        if (!waitingList.isEmpty()) {
            ...
            do {
                ...
                System.out.println("handleOutgoingMessages: " + msg);

                boolean sentGood = sendMessage(msg);
                System.out.println("handleOutgoingMessages send ret: " + sentGood);
